...
{
    public class ReportUserAboutDisablingShare8Job : IJob
    {
            ...
            using (DataContext _db = new DataContext())
            {
                ...
                using (DbContextTransaction tran = _db.Database.BeginTransaction(System.Data.IsolationLevel.Serializable))
                {
                    try
                    {
                        ...
                        {
                            message = _db.MessageTemplates.Where(m => m.Name == "OnShare8Disabling").FirstOrDefault();
                            message_geo = _db.MessageTemplates.Where(m => m.Name == "OnShare8Disabling_GEO").FirstOrDefault();
                            ...
                            foreach (var sbs in sbp)
                            {
                                ...
                                if (package.Id == 304084)
                                {
                                    ...
                                    {
                                        string messageText = String.Format(message.Desc, card.FinishDate.ToString("dd/MM/yyyy"), card.AbonentNum);
                        ...
                        Task.Run(async () => { await Utils.Utils.sendMessage("598733767", "დამთავრდა სმს გაგზავნა გათიშვამდე 2დღით ადრე 8-₾ პაკეთისთვის"); }).Wait();
                        Task.Run(async () => { await Utils.Utils.sendMessage("593668668", "დამთავრდა სმს გაგზავნა გათიშვამდე 2დღით ადრე 8-₾ პაკეთისთვის"); }).Wait();
                        Task.Run(async () => { await Utils.Utils.sendMessage("571711305", "დამთავრდა სმს გაგზავნა გათიშვამდე 2დღით ადრე 8-₾ პაკეთისთვის"); }).Wait();
